...
Rodar: python -m pytest test_grimoire_listen.py -q
"""
import array
from grimoire_listen import Endpointer, rms, separar_chamada, FRAME_AMOSTRAS, FRAME_BYTES


def test_chamada_com_nome_e_pedido():
    tem, resto = separar_chamada("Grimoire, que horas são")
    assert tem is True
    assert resto == "que horas são"


def test_chamada_com_saudacao():
    tem, resto = separar_chamada("ei Grimoire abre o firefox")
    assert tem is True
    assert resto == "abre o firefox"


def test_so_o_nome():
    tem, resto = separar_chamada("Grimoire")
    assert tem is True
    assert resto == ""
...
def test_variacoes_de_transcricao_do_nome():
    for t in ["grimório me ajuda", "Grimorio liga a luz", "grimoir roda o date"]:
        tem, _ = separar_chamada(t)
        assert tem is True


def test_variacoes_extras_de_pronuncia_brasileira():
    """Grafias que o Whisper produz para a pronúncia BR do nome (não a francesa)."""
    for t in ["grimoiri liga a luz", "grimoari que horas são", "grimoare abre o firefox",
              "grimuar liga a luz", "grimoá desliga o som", "grimuá qual é a previsão"]:
        tem, _ = separar_chamada(t)
        assert tem is True, f"não reconheceu: {t!r}"


def test_nome_no_meio_da_frase_e_reconhecido_e_removido():
    """O nome não precisa estar no começo — 'por favor Grimoire, liga a luz'."""
    tem, resto = separar_chamada("por favor grimoire liga a luz")
    assert tem is True
    assert resto == "por favor liga a luz"


def test_nome_no_fim_da_frase():
    tem, resto = separar_chamada("qual é a previsão do tempo grimoire")
    assert tem is True
    assert resto == "qual é a previsão do tempo"


def test_nome_quebrado_em_duas_palavras_gri_moar():
    """O Whisper às vezes separa o nome em duas palavras: 'gri moar'."""
    tem, resto = separar_chamada("gri moar que horas são")
    assert tem is True
    assert resto == "que horas são"


def test_nome_quebrado_em_duas_palavras_grimo_ire():
    tem, resto = separar_chamada("grimo ire liga a luz")
    assert tem is True
    assert resto == "liga a luz"


def test_nome_quebrado_em_duas_palavras_gri_moire():
    tem, resto = separar_chamada("gri moire liga a luz")
    assert tem is True
    assert resto == "liga a luz"


def test_nome_quebrado_em_duas_palavras_grimoi_re():
    tem, resto = separar_chamada("grimoi re liga a luz")
    assert tem is True
    assert resto == "liga a luz"


def test_nome_quebrado_em_tres_palavras_gri_mo_ar():
    """Quebra em TRÊS pedaços: 'gri' + 'mo' + 'ar' = 'grimoar'."""
    tem, resto = separar_chamada("gri mo ar liga a luz")
    assert tem is True
    assert resto == "liga a luz"


def test_nome_quebrado_no_meio_da_frase():
    tem, resto = separar_chamada("ei gri moar abre o firefox")
    assert tem is True
    assert resto == "abre o firefox"
...
def test_palavras_comuns_do_portugues_nao_acordam():
    ...
    Cada uma destas foi escolhida por parecer com 'Grimoire' de algum jeito:
    começa com 'gr' (grito, grilo, grande, gramática, grave, britânico tem 'r'
    no meio) ou tem sílabas parecidas (primeiro, memória, história, diretório,
    escritório, amor — todas com '-ório'/'-ória', que lembra 'grimório').
    """
    frases_comuns = [
        "que horas são",
        "eu ouvi um grito",
        "tinha um grilo na sala",
        "cheguei primeiro no trabalho",
        "não precisa gritar comigo",
        "isso ficou só na memória",
        "essa é uma história longa",
        "o arquivo está no diretório",
        "ele trabalha naquele escritório",
        "isso é fruto do amor",
        "a casa é muito grande",
        "ele é um cientista britânico",
        "preciso estudar gramática",
        "cuidado que o corte é grave",
    ]
...
def test_aquecer_mantem_o_pre_roll_durante_a_pausa():
    """Enquanto o Grimoire fala, a escuta fica pausada. Se o pre-roll for zerado,
