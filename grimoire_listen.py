#!/usr/bin/env python3
"""Grimoire — escuta contínua (saber quando você começou e parou de falar).
...
lá no grimoire.py, que usa este Endpointer.
"""
import array
import difflib
import math
...
class Endpointer:
    ...
    def reset(self):
        """Zera tudo (usado quando a escuta é pausada — enquanto o Grimoire fala)."""
        self._reset()
        self.pre = []
    ...
    def aquecer(self, frame: bytes) -> None:
        """Quadro que chega enquanto a escuta está pausada (o Grimoire está falando).
        ...
        if e <= max(self.limiar_abs, self.piso * 3.5):
            self._aprender_ruido(e)          # a voz do próprio Grimoire não vira "ruído de fundo"
...
# no meio do nome ("grim-o-ire", "grim-oa-r", "grim-ua-r") de graça, porque a
...
#   1. a palavra tem que COMEÇAR com "gr" — corta memória, história, amor,
#      diretório, escritório, britânico de cara, sem nem calcular distância;
#   2. o comprimento tem que ficar perto do nome real (5 a 10 letras) — corta
#      "grim" sozinho (curto demais para confirmar) e nomes muito compridos.
# Só depois dessas duas o limiar de parecença (0.80) decide. Esse número não é
# chute: medido contra a lista de palavras comuns do pedido, a pior confusão
# possível ("gritar") fica em 0,77 — e a pior grafia aceitável do nome
# ("grimwar", erro feio de transcrição) fica em 0,86. Sobra margem dos dois lados.
NOMES = ("grimoire", "grimório", "grimorio", "grimoir", "grimoar", "grimore",
         "grimoiri", "grimoari", "grimoare", "grimuar")
_SAUDACOES = ("ei ", "ê ", "ô ", "o ", "oi ", "olá ", "ola ", "alô ", "alo ")
_LIMIAR_PARECENCA = 0.80
_TAM_MIN, _TAM_MAX = 5, 10          # "grimoire" tem 8; damos folga pra menos e pra mais
_PALAVRA_RE = re.compile(r"[^\W\d_]+", re.UNICODE)   # sequência de letras (sem número/underline)


def _sem_acento(s: str) -> str:
    """Tira acento (grimório -> grimorio) para comparar sem depender da grafia exata."""
...
def _pontuar_nome(palavra: str, canon):
    """Quanto essa palavra (ou palavra colada) se parece com 'Grimoire'?
    ...
    português sem gastar SequenceMatcher, e sozinhas já eliminam "memória"/
    "amor"/"história" (não começam com "gr") e "grim" isolado (curto demais).
...
def separar_chamada(texto: str, nomes=NOMES):
    ...
    palavras, tipo "gri moar" ou "grimo ire"); resto = o pedido sem o nome nem
    ...
    # O Whisper pode quebrar "grimoire" em 2 ou até 3 "palavras" separadas
    # ("gri moar", "grimo ire", "gri mo ar"...). Por isso, em cada posição,
    ...
    # aparecem assim:
    #   - "maior primeiro" falha pra menos: em "grimo ire" o pedaço isolado
    #     "grimo" (prefixo de "grimoire") já passa sozinho no limiar; se ele
    #     vencesse por ser tentado primeiro, o "ire" sobraria colado no pedido.
    #   - "maior primeiro" falha pra mais também: em "gri moar que horas",
    #     colar as 3 primeiras palavras dá "grimoarque", que AINDA passa no
    #     limiar (a distância de edição tolera a palavra "que" grudada no
    #     fim) — e puxaria "que" para dentro do nome, comendo pedaço do pedido.
    # Comparando a pontuação, "grimoar" (pontuação 1.0, k=2) vence "grimoarque"
    # (pontuação ~0.82, k=3) e "grimo" (pontuação ~0.83, k=1) perde para
    # "grimoire" (pontuação 1.0, k=2) — o casamento mais "redondo" sempre ganha.
    span = None
    MAX_PEDACOS = 3
